...
func (p *Poset) _ancestor(x, y string) (bool, error) {
	...
	eyCreator := p.Participants[ey.Creator()]
...
func (p *Poset) _selfAncestor(x, y string) (bool, error) {
	...
	exCreator := p.Participants[ex.Creator()]
...
func (p *Poset) initEventCoordinates(event *Event) error {
	...
	creator := event.Creator()
	creatorID, ok := p.Participants[creator]
	if !ok {
...
func (p *Poset) createSelfParentRootEvent(ev Event) (RootEvent, error) {
	...
	selfParentRootEvent := RootEvent{
		Hash:             sp,
		CreatorID:        p.Participants[ev.Creator()],
		Index:            ev.Index() - 1,
			...
			for j := roundIndex + 1; j <= p.Store.LastRound(); j++ {
				for _, y := range p.Store.RoundWitnesses(j) {
					...
					if diff == 1 {
						...
						//normal round
						if math.Mod(float64(diff), float64(len(p.Participants))) > 0 {
...
func (p *Poset) GetFrame(roundReceived int) (Frame, error) {
	...
	//order roots
	orderedRoots := make([]Root, len(p.Participants))
	for i := 0; i < len(p.Participants); i++ {
		orderedRoots[i] = roots[p.ReverseParticipants[i]]
	}
...
func (p *Poset) ProcessSigPool() error {
	...
	for i, bs := range p.SigPool {
		...
		if _, ok := p.Participants[validatorHex]; !ok {
			p.logger.WithFields(logrus.Fields{
				"index":     bs.Index,
...
func (p *Poset) ReadWireInfo(wevent WireEvent) (*Event, error) {
	...
	creator := p.ReverseParticipants[wevent.Body.CreatorID]
	...
	if wevent.Body.SelfParentIndex >= 0 {
		selfParent, err = p.Store.ParticipantEvent(creator, wevent.Body.SelfParentIndex)
		if err != nil {
			return nil, err
		}
	}
	if wevent.Body.OtherParentIndex >= 0 {
		otherParentCreator := p.ReverseParticipants[wevent.Body.OtherParentCreatorID]
		otherParent, err = p.Store.ParticipantEvent(otherParentCreator, wevent.Body.OtherParentIndex)
		if err != nil {
