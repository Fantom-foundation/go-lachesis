...
func TestSocketProxyServer(t *testing.T) {
	...
	// and calling methods
	babbleProxy, err := bproxy.NewSocketLachesisProxy(proxyAddr, clientAddr, 1*time.Second, common.NewTestLogger(t))
...
func TestSocketProxyClient(t *testing.T) {
	...
	//create babble proxy
	babbleProxy, err := bproxy.NewSocketLachesisProxy(proxyAddr, clientAddr, 1*time.Second, logger)
