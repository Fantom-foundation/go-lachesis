...
func TestRestore(t *testing.T) {
	...
	for _, poset := range posets {
		...
		poset.callback.ApplyBlock = func(block *inter.Block, decidedFrame idx.Frame, cheaters inter.Cheaters) (bool, []common.Hash) {
			_, hh := applyBlock(block, decidedFrame, cheaters)
			return decidedFrame == idx.Frame(maxEpochBlocks), hh
		}
	}
